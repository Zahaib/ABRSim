...
k=1;for i in `ls ../../results/convivaData/group0/*.out`; do python simulation.py $i >> bb-group0-res.txt; ((k++)); if [ $k -eq 10000 ]; then break; fi; done
#k=1;for i in `ls ../../results/convivaData/group1/*.out`; do python simulation.py $i >> bb-group1-res.txt; ((k++)); if [ $k -eq 10000 ]; then break; fi; done
k=1;for i in `ls ../../results/convivaData/group2/*.out`; do  python simulation.py $i >> bb-group2-res.txt; ((k++)); if [ $k -eq 10000 ]; then break; fi; done
k=1;for i in `ls ../../results/convivaData/group3/*.out`; do  python simulation.py $i >> bb-group3-res.txt; ((k++)); if [ $k -eq 10000 ]; then break; fi; done
