...
class globalstate(object):
  ...
  def doNonConditional(self, config):
    if self.clock + config.simstep > self.traceSessiontime:
      self.simstep = self.traceSessiontime - self.clock
    self.clock += config.simstep
...
class chunkstate(object):
  ...
  def doNonConditional(self, config):
    self.chd_thisStep = 0
    self.chunk_sched_time_delay = max(0, self.chunk_sched_time_delay - config.simstep)
  ...
  def doConditional(self, config, bwArray, globalstate, chunkstate, bitratestate, bandwidthstate, bufferstate):
    ...
    if self.chunk_sched_time_delay < config.simstep:
      numChunks, completionTimeStamps, self.chunk_sched_time_delay = helpers.chunksDownloaded(globalstate.clock - config.simstep, globalstate.clock, bitratestate.bitrate, \
      ...
      if int(self.chd_thisStep) >= 1 and self.chunk_sched_time_delay < config.simstep:
...
class bufferstate(object):
  ...
  def doNonConditional(self, config, isComplete):
    if self.blen > 0:
      self.buffering = False
  ...
  def doConditional(self, config, bwArray, globalstate, chunkstate, bitratestate, bandwidthstate, bufferstate):
    ...
    if self.buffering:
      ...
      chunkstate.chunk_residue_step_start, chunkstate.chunks_downloaded_step_start), config.simstep/config.MSEC_IN_SEC)
    ...
    if not self.buffering and self.blen >= 0 and self.blen + self.blenAdded_thisStep < config.simstep/config.MSEC_IN_SEC:
      self.playStalled_thisStep += (config.simstep/config.MSEC_IN_SEC - self.blen - self.blenAdded_thisStep)
      self.buffering = True
    ...
    if self.buffering:
      ...
      self.blen = max(0, self.blen  - config.simstep/config.MSEC_IN_SEC)
    else:
      self.blen = max(0, chunkstate.chunks_downloaded * config.chunksize - (globalstate.playtime + config.simstep/config.MSEC_IN_SEC - self.playStalled_thisStep))
